...
def generate_images(prompt, randomize=False):
    ...
    logger.info(f"[MV-ADAPTER] generating images. prompt to be refined: {prompt}, seed: {seed}")

    refined_prompt, negative_prompt = llm_prompt_processing(prompt)
    if negative_prompt == None:
        negative_prompt = "nothing to change"
    ...
    llm_avaliation, llm_eval_neg_prompt = check_views_quality(views_dir, refined_prompt)
    return (
        paths, views_dir,
        gr.update(visible=True),
        refined_prompt,
        gr.update(value=refined_prompt, visible=True),
        gr.update(value=negative_prompt, visible=True),
